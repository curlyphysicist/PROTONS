...
JSONparam = "../models/params/FINAL_EVENTS/PPh-%sh-%sMeV.json" % (time, channels)       # saving the model
H5bestparam = "../models/params/FINAL_EVENTS/PPh-best-%sh-%sMeV.h5" % (time, channels)  # saving the weights of the BEST model

file = '../прогнозы/FINAL_EVENTS/PPh-%sMeV-%sh.csv' % (channels, time)      # PPh15 - protons, parameters, hours, 15min.
#info = '../прогнозы/FINAL_EVENTS/PPh-%sMeV-%sh.txt' % (channels, time)   # Для записи статистики в процессе обучения
...
#----------------------------------------------------------------------------------------------#
# P + PARAMS      HOURS                 =   42 ВХОДА
array = []                              # массив из номеров столбцов, которые пойдут в массивы обучающих данных
cnt = 0; i = 0; w = 0
for i in range (0,6):
    for cnt in range(32,36):            # без учёта данных за "сейчас"
        array.append(cnt + 27*i)
    for w in range (1,4):
        array.append(35 + 5*w + 27*i)
#----------------------------------------------------------------------------------------------#
...
print(array)                                  # Проверяй, чтобы столбцы, которые ты прогнозируешь, не появлялись в обучающих ;)
print(aim)
print(train[array])
print(train[aim])
...
model = Sequential()
model.add(Dense(80, input_dim=42, kernel_initializer='normal', activation='sigmoid'))
