...
for name in files:
    ...
    fread = open(name)      # хочу зачитать каждый файл, содрать с него шапку и подклеить к единому
    lines = {}               
    flag = '0'
    i = 0
    for line in fread:
    ...
    for line in lines:
        ...
        newarray.append(array[6]+','+array[8]+','+array[10]+','+array[14]+','+array[20]+','+array[22]+','+array[24]+','+array[28]) # инт с 2 детекторов
...
pd.DataFrame.to_csv(GOESdataframe, 'GOESdataafter2011.csv', index=False, index_label=False, decimal=',')
